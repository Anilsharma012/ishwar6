...
export default function CategoryProperties() {
  ...
  const getCurrentCategory = () => {
    // First check query params (passed by pages like /buy/:slug?category=buy)
    ...
    if (path.startsWith("/agricultural/")) return "agricultural";
    return category;
  };
  ...
  const fetchProperties = async () => {
    try {
      ...
      // ✅ NEW: query params se direct filters (listings-style)
      const qpCategory = searchParams.get("category") || "";
      const qpSubCategory =
        searchParams.get("subCategory") || searchParams.get("subcategory") || "";
      const qpMini = searchParams.get("miniSubcategory") || "";

      // Handle category and subcategory from URL
      const currentCategory = getCurrentCategory();

      // Send category to API to ensure consistent filter logic
      const finalCategory = qpCategory || currentCategory;
      if (finalCategory && finalCategory !== "all") {
        params.append("category", finalCategory);
      }

      // ✅ priceType based on main category OR user filter override
      // (filter dropdown "Property For" should work)
      if (filters.priceType) {
        params.append("priceType", filters.priceType);
      } else {
        if (finalCategory === "buy" || finalCategory === "sale") {
          params.append("priceType", "sale");
        } else if (finalCategory === "rent") {
          params.append("priceType", "rent");
        } else if (finalCategory === "lease") {
          params.append("priceType", "lease");
        } else if (finalCategory === "pg") {
          params.append("propertyType", "pg");
        }
      }

      // ✅ propertyType direct query support
      const qpPropertyType = searchParams.get("propertyType") || "";
      if (qpPropertyType) {
        params.append("propertyType", qpPropertyType);
      }

      // Handle propertyType from subcategory parameter (e.g., /buy/residential)
      if (subcategory && !slug && !qpPropertyType) {
        params.append("propertyType", subcategory);
      }

      // Handle slug mapping (e.g., /buy/1bhk)
      if (slug && !qpPropertyType) {
        const mapping = getPropertyTypeAndSubCategory(finalCategory, slug);
      ...
      // Handle direct propertyType from category (when category is like "commercial/agricultural")
      if (
        category &&
      ...
      if (propertyType && !qpPropertyType) params.append("propertyType", propertyType);

      // ✅ NEW: subCategory/miniSubcategory query support (this is what was missing)
      if (qpSubCategory) {
        params.set("subCategory", qpSubCategory);
      }
      if (qpMini) {
        params.set("miniSubcategory", qpMini);
      }
      ...
      // ✅ FIX: apiResponse.json() ko await karo (pehle data parse hi nahi ho raha tha)
      const apiResponse = await (window as any).api(
        `properties?${params.toString()}`,
      );

      const data =
        apiResponse?.ok && typeof apiResponse.json === "function"
  ...
  const clearFilters = () => {
    ...
    // ✅ FIX: category/subCategory/miniSubcategory etc. ko mat hatao
    const params = new URLSearchParams(searchParams);
  ...
  const getCategoryTitle = () => {
    const currentCategory = getCurrentCategory();
    const qpMini = searchParams.get("miniSubcategory");
    const qpSub = searchParams.get("subCategory") || searchParams.get("subcategory");

    // ✅ If using listings-style params, show better title
    if (qpMini) {
      const niceMini = qpMini
        .replace(/-/g, " ")
        .replace(/\b\w/g, (l) => l.toUpperCase());
      const niceSub = qpSub
        ? qpSub.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())
        : "";
      const niceCat = (searchParams.get("category") || currentCategory || "")
        .replace(/-/g, " ")
        .replace(/\b\w/g, (l) => l.toUpperCase());

      return niceSub
        ? `${niceMini} • ${niceSub} • ${niceCat}`
        : `${niceMini} • ${niceCat}`;
    }
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              <div>
                <div className="flex items-center gap-2 text-xs text-gray-500 mb-1">
                  <span className="capitalize">
                    {searchParams.get("category") || getCurrentCategory() || ""}
                  </span>
                    ...
                    >
                      <img
                        src={
                          ...
                          ((property as any).images?.[0] as any) ??
                          "/placeholder.png"
                        }
                        alt={property.title}
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          const images = Array.isArray((property as any).images)
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          {(property as any).priceType === "rent" && "/month"}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center text-gray-500 mb-3 text-sm">
                        {(property as any).specifications?.bedrooms && (
                          <span className="mr-4">
                            {(property as any).specifications.bedrooms} BHK
                          </span>
                        )}
                        {(property as any).specifications?.bathrooms && (
                          <span className="mr-4">
                            {(property as any).specifications.bathrooms} Bath
                          </span>
                        )}
                        <span>
                          {(property as any).specifications?.area || 0} sq ft
                        </span>
                      </div>

                      <div className="flex items-center justify-between">
                        <span className="text-xs text-gray-400">
                          {(property as any).contactInfo?.name || "Owner"}
                        </span>
                        <Button
                          onClick={(e) => {
                            e.stopPropagation();
      ...
      {selectedPropertyForZoom && (
        <ImageModal
          ...
          images={
            Array.isArray((selectedPropertyForZoom as any).images)
              ? (selectedPropertyForZoom as any).images
                  .map((img: any) =>
                    typeof img === "string" ? img : img?.url,
                  )
                  .filter(Boolean)
