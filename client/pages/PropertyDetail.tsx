...
function LightboxModalZoom({
  ...
  const wheel: React.WheelEventHandler<HTMLDivElement> = (e) => {
    ...
    // zoom ko thoda limit rakha hai taaki pixels jyada na faaten
...
/>
        ...
        <div className="absolute bottom-4 left-0 right-0 text-center text-xs text-gray-300 px-4">
          Tap / click to zoom · Scroll to zoom · Drag to move · ESC to close
        </div>
      </div>
    </div>
  );
...
export default function PropertyDetail() {
  ...
  const toggleLike = async () => {
    ...
    try {
      setLikeBusy(true);
      setIsLiked((p) => !p);
      const res = await apiWrite(`favorites/${id}`, isLiked ? "DELETE" : "POST");
