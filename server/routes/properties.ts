  ...
  try {
    ...
    if (miniSubcategory && !miniSubcategoryId) {
      try {
        ...
        // remains correct. (This keeps previous behavior intact.)
        if (!parentCategorySlug) {
          parentCategorySlug = normalizedPriceType === "rent" ? "rent" : "buy";
        }
        ...
        if (subcategoryDoc) {
          ...
          if (miniDoc) {
            ...
            console.log("⚠️ mini-subcategory not found for", {
              slug: normalizedMiniSlug,
              subcategoryId: subcategoryDoc._id?.toString(),
            });
          }
        } else {
          console.log("⚠️ subcategory not found for", {
            slug: normalizedSubCat,
            parentCategory: parentCategorySlug,
          });
        }
